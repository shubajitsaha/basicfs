...
////Device Driver level//////////////////////
struct direntry* seekFile(int fhandle,char *fname);
...
int main(int argc,char *argv[])
{
	...
	{
		scanf("%s",buf);
		struct direntry *de=seekFile(0,buf);
		if(de != NULL)
		{
	...
	{
		scanf("%s",buf);
		int handle=openFile(0,buf,0,0,0);
		if(handle > -1)
		{
	...
	else if(strcmp(buf,"read") == 0)
	{
		...
		ret=writeFile(handle,readbuf,size);
		if(ret > -1)
		{
			printf("success file Writing. Read string: %s %d\n",readbuf,ret);
		}
		else
		{
			perror("error.File Writing error\n");
...
int createFile(int dirhandle, char *fname, int mode, int uid, int gid)
{
	...
	//////////////////////////////////////////////////
	struct direntry *ret=seekFile(dirhandle,fname);
	if( ret != NULL)
	{
...
struct direntry* seekFile(int dirhandle,char *fname)
{
	int i,j;
...
int openFile(int dirhandle,char *fname,int mode, int uid, int gid)
{
	struct direntry *de=seekFile(dirhandle,fname);
	// if(de!=NULL)
	// {
	// 	printf("xxxxxxxxx %d",de->d_ino);
	// }
	// else printf("ball\n");
...
{
	int inodeindex=obbuffer[fhandle].ofo_inode;
	...
	{
		j=0;
		lseek(devfd,genearteDataBlockAdress(inodetable[inodeindex].i_blks[cnt++]),SEEK_SET);
		write(devfd,fbuff,min(nbytes,BLOCKSIZE));
		while(i<min(nbytes,BLOCKSIZE))
			buf[i++]=fbuff[j++];
		size+=min(nbytes,BLOCKSIZE);
		nbytes-=BLOCKSIZE;
	}
	return size;
}

int writeFile(int fhandle, char buf[], int nbytes) //fhandle = open oject buufer index
{
	int inodeindex=obbuffer[fhandle].ofo_inode;
...
int closeFile(int dirhandle,int fhandle) //fhandle = open oject buufer index
{
